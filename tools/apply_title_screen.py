...
if anchor not in s: raise SystemExit('title const anchor failed')
...
if anchor not in s: raise SystemExit('title state anchor failed')
...
if anchor not in s: raise SystemExit('title ready anchor failed')
...
if anchor not in s: raise SystemExit('title key input anchor failed')
s = s.replace(anchor, anchor + '\tif title_screen_active:\n\t\ttitle_screen_active = false\n\t\tqueue_redraw()\n\t\treturn\n', 1)
anchor = 'func _input(event: InputEvent) -> void:\n'
if anchor not in s: raise SystemExit('title pointer input anchor failed')
...
if anchor not in s: raise SystemExit('title draw anchor failed')
...
exec(Path('tools/apply_inventory_20_typefix.py').read_text(encoding='utf-8'), {})
